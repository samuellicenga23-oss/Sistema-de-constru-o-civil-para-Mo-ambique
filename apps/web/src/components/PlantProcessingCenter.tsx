...
export default function PlantProcessingCenter() {
  ...
  return (
    <aside
      className="fixed bottom-20 right-3 z-50 w-[min(25rem,calc(100vw-1.5rem))] cursor-pointer rounded-xl border border-slate-200 bg-white p-4 shadow-xl md:bottom-5 md:right-5"
      aria-live="polite"
      role="status"
      ...
      <div className="flex items-start gap-3">
        ...
        <button
          ...
          onClick={(event) => {
            ...
            dismissPlantProcessingTask(task.plantId);
          }}
      ...
      {!finished && !failed && (
        <div className="mt-3 flex items-center gap-3">
          <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-slate-100">
            <div className="h-full rounded-full bg-brand-600" style={{ width: `${task.progress.processingProgress}%` }} />
          </div>
          <span className="text-xs tabular-nums font-semibold text-slate-700">{task.progress.processingProgress}%</span>
        </div>
      )}
