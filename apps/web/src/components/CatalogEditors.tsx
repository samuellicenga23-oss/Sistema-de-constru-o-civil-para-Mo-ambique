    ...
    <Modal title={item ? "Ficha de mão-de-obra" : "Nova categoria de mão-de-obra"} subtitle="Salário, encargos, horas produtivas e vigência do custo." onClose={onClose} maxWidth="max-w-3xl">
      <form onSubmit={submit} className="space-y-5">
        <div className="rounded-xl border border-blue-100 bg-blue-50 px-4 py-3 flex items-center justify-between gap-4">
          ...
          <strong className="text-xl text-blue-950 tabular-nums">{loadedRate.toLocaleString("pt-MZ", { maximumFractionDigits: 2 })} MZN/h</strong>
        </div>
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <Field label="Salário mensal"><input required min="0.01" type="number" step="any" className="input" value={form.monthlySalary} onChange={(e) => setForm({ ...form, monthlySalary: e.target.value })} /></Field>
          <Field label="Horas produtivas/mês" hint="Se ficar vazio, usa os dias e horas de trabalho definidos na empresa."><input min="0.01" type="number" step="any" className="input" value={form.productiveHoursPerMonth} onChange={(e) => setForm({ ...form, productiveHoursPerMonth: e.target.value })} placeholder="176" /></Field>
          <Field label="Encargos sociais (%)"><input min="0" max="200" type="number" step="any" className="input" value={form.socialChargesPct} onChange={(e) => setForm({ ...form, socialChargesPct: e.target.value })} /></Field>
          <Field label="Custos complementares (%)" hint="EPI, ferramentas, transporte, alimentação ou outros custos não salariais."><input min="0" max="200" type="number" step="any" className="input" value={form.complementaryCostsPct} onChange={(e) => setForm({ ...form, complementaryCostsPct: e.target.value })} /></Field>
    ...
    <Modal title={item ? "Ficha técnica do material" : "Novo material"} subtitle="Especificação, unidade de medição, compra, perdas e rastreabilidade do preço." onClose={onClose} maxWidth="max-w-4xl">
      <form onSubmit={submit} className="space-y-5">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <Field label="Preço base (MZN)"><input required min="0" type="number" step="any" className="input" value={form.baseUnitCost} onChange={(e) => setForm({ ...form, baseUnitCost: e.target.value })} /></Field>
          ...
          <Field label="Perda padrão (%)" hint="Sugestão ao adicionar o material a uma composição; pode ser ajustada por serviço."><input min="0" max="100" type="number" step="any" className="input" value={form.defaultWastePct} onChange={(e) => setForm({ ...form, defaultWastePct: e.target.value })} /></Field>
          <Field label="Factor de importação"><input min="0.0001" type="number" step="any" className="input" value={form.importFactor} onChange={(e) => setForm({ ...form, importFactor: e.target.value })} /></Field>
          ...
          <Field label="Conteúdo da embalagem"><input min="0.0001" type="number" step="any" className="input" value={form.purchasePackageQty} onChange={(e) => setForm({ ...form, purchasePackageQty: e.target.value })} placeholder="50" /></Field>
        </div>
        <Field label="Especificação técnica"><textarea className="input min-h-20" value={form.specification} onChange={(e) => setForm({ ...form, specification: e.target.value })} placeholder="Classe, resistência, dimensões, norma, acabamento ou marca de referência" /></Field>
  ...
  return (
    <Modal title={item ? "Configurar zona de preço" : "Nova zona de preço"} subtitle="Ajustes geográficos aplicados apenas quando não existe um preço específico do recurso para a zona." onClose={onClose} maxWidth="max-w-3xl">
      <form onSubmit={submit} className="space-y-5">
        ...
        <div className="rounded-xl border border-amber-100 bg-amber-50 p-4"><p className="text-sm font-semibold text-amber-950 mb-3">Factores de localização sobre o preço base</p><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><Field label="Materiais (%)"><input type="number" step="any" className="input" value={form.materialAdjustmentPct} onChange={(e) => setForm({ ...form, materialAdjustmentPct: e.target.value })} /></Field><Field label="Transporte (%)"><input min="0" type="number" step="any" className="input" value={form.defaultTransportPct} onChange={(e) => setForm({ ...form, defaultTransportPct: e.target.value })} /></Field><Field label="Mão-de-obra (%)"><input type="number" step="any" className="input" value={form.labourAdjustmentPct} onChange={(e) => setForm({ ...form, labourAdjustmentPct: e.target.value })} /></Field><Field label="Equipamento (%)"><input type="number" step="any" className="input" value={form.equipmentAdjustmentPct} onChange={(e) => setForm({ ...form, equipmentAdjustmentPct: e.target.value })} /></Field></div></div>
