...
import { PAYMENT_DETAILS, formatMzn } from "../commercialPlans";
  ...
  return (
    <Modal title={`Activar plano ${planLabel}`} subtitle="Pague pelos dados abaixo e anexe o comprovativo" onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="rounded-lg border border-slate-200 p-3 text-xs leading-relaxed">
          <p className="mb-2 font-bold text-slate-900">Dados para pagamento — titular: {PAYMENT_DETAILS.holder}</p>
          <div className="grid gap-1.5 sm:grid-cols-2">
            {PAYMENT_DETAILS.banks.map((b) => (
              <div key={b.name} className="rounded bg-slate-50 p-2">
                <p className="font-semibold text-slate-800">{b.name}</p>
                <p className="text-slate-600">Conta: {b.account}</p>
                {"nib" in b && b.nib && <p className="text-slate-600">NIB: {b.nib}</p>}
              </div>
            ))}
            {PAYMENT_DETAILS.mobileMoney.map((m) => (
              <div key={m.name} className="rounded bg-slate-50 p-2">
                <p className="font-semibold text-slate-800">{m.name}</p>
                <p className="text-slate-600">Número: {m.number}</p>
              </div>
            ))}
          </div>
        </div>
