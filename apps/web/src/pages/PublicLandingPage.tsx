...
function HeroProduct() {
  return <div className="relative mx-auto w-full max-w-[650px] lg:ml-auto">
    ...
    <div className="overflow-hidden rounded-[24px] border border-slate-700 bg-[#142033] p-2.5 shadow-[0_34px_80px_rgba(20,32,51,.28)]">
      <div className="overflow-hidden rounded-[17px] bg-[#f8f9fb]">
        ...
        <div className="grid gap-3 p-3 sm:grid-cols-[1fr_1.7fr] sm:p-4">
          ...
          <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">
            ...
            ].map((row) => <div key={row.code} className={`grid grid-cols-[1.15fr_.85fr] items-center border-b border-slate-100 px-3 last:border-0 ${row.summary ? "bg-slate-50 py-2.5" : "py-2"}`}><div className={row.summary ? "" : "pl-4 border-l border-slate-200"}><p className={`truncate text-[11px] ${row.summary ? "font-display font-black" : "font-semibold"}`}><span className={`mr-2 ${row.summary ? "text-slate-500" : "text-[#0f8a90]"}`}>{row.code}</span>{row.name}</p><p className="mt-0.5 text-[8px] text-slate-400">{row.sub}</p></div><div className="relative h-5 border-l border-slate-100"><span className={`absolute top-1.5 overflow-hidden ${row.summary ? "h-1.5 rounded-sm bg-slate-400" : "h-2.5 rounded bg-[#d5f5f6]"}`} style={{left:row.left,width:row.width}}><i className={`block h-full ${row.summary ? "bg-[#142033]" : "bg-[#ed6c22]"}`} style={{width:row.done}} /></span></div></div>)}
          </div>
        </div>
      </div>
    </div>
...
function ProductPreview({ active }: { active: ProductKey }) {
  if (active === "orcamento") return <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_24px_60px_rgba(20,32,51,.09)]">
    ...
    {[{code:"03.01",name:"Betão C25/30 em fundações",qty:"46,2 m³",total:"612.840",tag:"Composição"},{code:"03.02",name:"Aço A500 nervurado",qty:"3.840 kg",total:"468.480",tag:"3 cotações"},{code:"03.03",name:"Cofragem para sapatas",qty:"284 m²",total:"198.800",tag:"Zona Maputo"}].map((row) => <div key={row.code} className="grid grid-cols-[minmax(0,1fr)_84px_108px] items-center border-b border-slate-100 px-5 py-4 last:border-0"><div><p className="text-xs font-bold"><span className="mr-2 text-[#0f8a90]">{row.code}</span>{row.name}</p><span className="mt-1 inline-block rounded bg-orange-50 px-2 py-0.5 text-[9px] font-bold text-[#a84a16]">{row.tag}</span></div><span className="text-xs text-slate-600">{row.qty}</span><strong className="text-right text-xs">{row.total} MZN</strong></div>)}
...
export default function PublicLandingPage() {
  ...
  return <div className="min-h-screen overflow-x-hidden bg-[#f6f8f8] text-[#142033] selection:bg-[#bdeef0]">
    <header className="fixed inset-x-0 top-0 z-50 border-b border-[#142033]/10 bg-[#f6f8f8]/90 backdrop-blur-md">
    ...
    <main id="inicio" className="pt-[76px]">
      <section className="relative overflow-hidden border-b border-[#142033]/10">
        <div aria-hidden className="pointer-events-none absolute inset-0">
          <div className="absolute -left-24 top-0 h-[520px] w-[520px] rounded-full bg-[radial-gradient(circle,rgba(26,173,180,0.18),transparent_68%)]" />
          <div className="absolute -right-16 top-10 h-[460px] w-[460px] rounded-full bg-[radial-gradient(circle,rgba(237,108,34,0.14),transparent_70%)]" />
