...
export default function MeasurementCertificatePage() {
  ...
  async function saveLine(line: MeasurementLine) {
    ...
    if (!draft) return;
    const periodQty = line.hasFieldMemory ? line.periodQty : Number(draft.periodQty || 0);
  ...
  async function saveAllDirty() {
    ...
    const toSave = data.lines.filter((line) => {
      ...
      const qtyDirty = !line.hasFieldMemory && Number(draft.periodQty || 0) !== line.periodQty;
      return qtyDirty || (draft.notes || "") !== (line.notes ?? "") || (draft.overrunReason || "") !== (line.overrunReason ?? "");
    });
