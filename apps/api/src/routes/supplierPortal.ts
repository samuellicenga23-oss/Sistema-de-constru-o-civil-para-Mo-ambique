...
export async function supplierPortalRoutes(app: FastifyInstance) {
  ...
  const marketplaceMaterialSchema = z.object({ materialId: z.string().uuid(), unitCost: z.number().nonnegative(), currency: z.enum(CURRENCIES).default("MZN") });
  const marketplaceLabourSchema = z.object({ labourCategoryId: z.string().uuid(), hourlyCost: z.number().nonnegative(), currency: z.enum(CURRENCIES).default("MZN") });
  const marketplaceEquipmentSchema = z.object({ equipmentId: z.string().uuid(), hourlyCost: z.number().nonnegative(), currency: z.enum(CURRENCIES).default("MZN") });

  app.get("/api/supplier/marketplace/materials", { preHandler: requireSupplierAuth }, async (request, reply) => {
    const supplier = await ownGlobalSupplier(supplierAccountIdOf(request));
    if (!supplier) return reply.code(404).send({ error: "Ficha de marketplace não encontrada" });
    const rows = await db
      .select({ price: supplierMaterialPrices, materialName: materials.name, unit: materials.unit })
    ...
    return rows.map((r) => ({ ...r.price, materialName: r.materialName, unit: r.unit }));
  });
  ...
  app.put("/api/supplier/marketplace/materials", { preHandler: requireSupplierAuth }, async (request, reply) => {
    ...
    const [existing] = await db
      .select()
      .from(supplierMaterialPrices)
      .where(and(eq(supplierMaterialPrices.supplierId, supplier.id), eq(supplierMaterialPrices.materialId, materialId), isNull(supplierMaterialPrices.zoneId)))
      .limit(1);
    if (existing) {
    ...
    const [row] = await db.insert(supplierMaterialPrices).values({ supplierId: supplier.id, materialId, unitCost: unitCost.toString(), currency }).returning();
  ...
  app.get("/api/supplier/marketplace/labour", { preHandler: requireSupplierAuth }, async (request, reply) => {
    ...
    if (!supplier) return reply.code(404).send({ error: "Ficha de marketplace não encontrada" });
    const rows = await db
      .select({ price: supplierLabourPrices, labourName: labourCategories.name })
      .from(supplierLabourPrices)
      .innerJoin(labourCategories, eq(supplierLabourPrices.labourCategoryId, labourCategories.id))
      .where(and(eq(supplierLabourPrices.supplierId, supplier.id), isNull(supplierLabourPrices.zoneId)));
    return rows.map((r) => ({ ...r.price, labourName: r.labourName }));
  });
  ...
  app.put("/api/supplier/marketplace/labour", { preHandler: requireSupplierAuth }, async (request, reply) => {
    ...
    const [existing] = await db
      .select()
      .from(supplierLabourPrices)
      .where(and(eq(supplierLabourPrices.supplierId, supplier.id), eq(supplierLabourPrices.labourCategoryId, labourCategoryId), isNull(supplierLabourPrices.zoneId)))
      .limit(1);
    if (existing) {
    ...
    const [row] = await db.insert(supplierLabourPrices).values({ supplierId: supplier.id, labourCategoryId, hourlyCost: hourlyCost.toString(), currency }).returning();
    return reply.code(201).send(row);
  });
  ...
  app.get("/api/supplier/marketplace/equipment", { preHandler: requireSupplierAuth }, async (request, reply) => {
    ...
    if (!supplier) return reply.code(404).send({ error: "Ficha de marketplace não encontrada" });
    const rows = await db
      .select({ price: supplierEquipmentPrices, equipmentName: equipment.name })
      .from(supplierEquipmentPrices)
      .innerJoin(equipment, eq(supplierEquipmentPrices.equipmentId, equipment.id))
      .where(and(eq(supplierEquipmentPrices.supplierId, supplier.id), isNull(supplierEquipmentPrices.zoneId)));
    return rows.map((r) => ({ ...r.price, equipmentName: r.equipmentName }));
  });
  ...
  app.put("/api/supplier/marketplace/equipment", { preHandler: requireSupplierAuth }, async (request, reply) => {
    ...
    const [existing] = await db
      .select()
      .from(supplierEquipmentPrices)
      .where(and(eq(supplierEquipmentPrices.supplierId, supplier.id), eq(supplierEquipmentPrices.equipmentId, equipmentId), isNull(supplierEquipmentPrices.zoneId)))
      .limit(1);
    if (existing) {
    ...
    const [row] = await db.insert(supplierEquipmentPrices).values({ supplierId: supplier.id, equipmentId, hourlyCost: hourlyCost.toString(), currency }).returning();
    return reply.code(201).send(row);
  });
