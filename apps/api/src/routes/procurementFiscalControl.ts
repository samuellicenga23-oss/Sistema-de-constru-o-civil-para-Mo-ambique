...
export async function procurementFiscalControlRoutes(app: FastifyInstance) {
  ...
  app.post("/api/payment-requests/:id/submit", { preHandler: financePermission }, async (request, reply) => {
    ...
    const [updated] = await db.update(procurementPaymentRequests).set({ status: "submetido", submittedAt: new Date(), updatedAt: new Date() }).where(eq(procurementPaymentRequests.id, id)).returning(); return updated;
  });
