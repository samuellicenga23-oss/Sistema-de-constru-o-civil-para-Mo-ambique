...
export async function notifyUser(userId: string, title: string, body: string, link?: string): Promise<void> {
  try {
    await db.insert(notifications).values({ userId, title, body, link: link ?? null });
  } catch {
    // Silencioso de propósito — ver comentário acima.
  }
}

export async function notifyUsers(userIds: string[], title: string, body: string, link?: string): Promise<void> {
  if (!userIds.length) return;
  try {
    await db.insert(notifications).values(userIds.map((userId) => ({ userId, title, body, link: link ?? null })));
  } catch {
