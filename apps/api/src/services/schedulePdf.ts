...
const money = (value: number, currency: string) => new Intl.NumberFormat("pt-PT", { style: "currency", currency, minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(value);
...
export function buildScheduleHtml(
  ...
  schedule: Schedule,
  print: ResolvedSchedulePrintOptions,
  brand?: { displayName: string; logoDataUri: string | null; primaryColor: string; footer: string },
) {
  const accent = brand?.primaryColor || "#ef6c23";
  const brandName = brand?.displayName || "Empresa";
  const logo = brand?.logoDataUri;
  const footerText = brand?.footer || `Documento emitido por ${brandName}`;
  ...
  const brandMark = logo
    ...
    : `<div class="mark" style="background:${esc(accent)}">${esc(brandName.slice(0, 1).toUpperCase())}</div>`;
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><style>
    ...
    header{display:flex;justify-content:space-between;align-items:flex-start;border-bottom:2px solid ${esc(accent)};padding-bottom:5mm;margin-bottom:4mm;break-inside:avoid}.brand{display:flex;gap:3mm;align-items:center}.mark{width:11mm;height:11mm;border-radius:3mm;color:white;font:bold 17px Arial;display:grid;place-items:center}.logo{max-height:12mm;max-width:28mm;object-fit:contain}.brand h1{font-size:16px;letter-spacing:.5px;margin:0}.brand p,.meta p{margin:1mm 0;color:#667085}.title{text-align:center}.title h2{font-size:16px;margin:0 0 1mm;color:${esc(accent)}}.title p{margin:0;color:#667085}.meta{text-align:right}
    .metrics{display:flex;gap:4mm;margin-bottom:4mm;break-inside:avoid}.metric{border:1px solid #d9e0ea;border-radius:2mm;padding:2.5mm 4mm;min-width:38mm}.metric span{display:block;color:#667085;text-transform:uppercase;font-size:7px;letter-spacing:.5px}.metric strong{font-size:13px}.progress{height:2mm;background:#e7ebf1;border-radius:5px;overflow:hidden;margin-top:1mm}.progress i{display:block;height:100%;background:${esc(accent)}}
    ...
    <footer><span>${esc(footerText)}</span><span>${print.paper} paisagem · escala ${print.scalePercent}%</span></footer>
  </body></html>`;
}
...
export async function buildSchedulePdf(
  ...
  try {
    ...
    await page.setContent(buildScheduleHtml(project, schedule, options, brand), { waitUntil: "networkidle0" });
