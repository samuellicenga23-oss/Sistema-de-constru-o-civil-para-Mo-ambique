...
export function buildScheduleHtml(project: ProjectHeader, schedule: Schedule, print: ResolvedSchedulePrintOptions) {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><style>
    ...
    <header><div class="brand"><div class="mark">S</div><div><h1>SIGO</h1><p>SISTEMA INTEGRADO DE GESTÃO DE OBRAS</p></div></div><div class="title"><h2>CRONOGRAMA DE EXECUÇÃO</h2><p>${esc(project.name)}${project.client ? ` · Dono da obra: ${esc(project.client)}` : ""}</p></div><div class="meta"><strong>Planeamento base</strong><p>${dateLabel(schedule.startDate!)} — ${dateLabel(schedule.endDate!)}</p><p>Actualizado em ${new Intl.DateTimeFormat("pt-PT", { dateStyle: "medium" }).format(new Date())}</p></div></header>
    ...
    <footer><span>SIGO · Cronograma integrado ao orçamento, Diário de Obra e Autos de Medição</span><span>${print.paper} paisagem · escala ${print.scalePercent}% · WBS e progresso real</span></footer>
  </body></html>`;
}
