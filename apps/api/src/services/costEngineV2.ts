...
async function loadBaseResourceFacts(compositionId: string, requestingCompanyId: string | null, zoneId: string | null | undefined, client: Client) {
  ...
  const [labourCandidates, materialCandidates, equipmentCandidates] = await Promise.all([
    labourFamilies.length ? client.select({ id: labourCategories.id, familyKey: labourCategories.familyKey, name: labourCategories.name, companyId: labourCategories.companyId, hourlyRate: labourCategories.hourlyRate, currency: labourCategories.currency }).from(labourCategories).where(and(inArray(labourCategories.familyKey, labourFamilies), companyScope(labourCategories.companyId, requestingCompanyId))) : Promise.resolve([]),
    materialFamilies.length ? client.select({ id: materials.id, familyKey: materials.familyKey, name: materials.name, companyId: materials.companyId, unit: materials.unit, baseUnitCost: materials.baseUnitCost, importFactor: materials.importFactor, includesVat: materials.includesVat, currency: materials.currency, purchasePackageLabel: materials.purchasePackageLabel, purchasePackageQty: materials.purchasePackageQty }).from(materials).where(and(inArray(materials.familyKey, materialFamilies), companyScope(materials.companyId, requestingCompanyId))) : Promise.resolve([]),
  ...
  if (zoneId && zone && resolvedMaterials.size) {
    ...
    const rows = await client.select({ materialId: materialZonePrices.materialId, unitCost: materialZonePrices.unitCost, includesVat: materialZonePrices.includesVat })
      .from(materialZonePrices).where(and(eq(materialZonePrices.zoneId, zoneId), inArray(materialZonePrices.materialId, ids)));
    zoneCostByMaterialId = new Map(rows.map((row) => [row.materialId, { unitCost: Number(row.unitCost), includesVat: row.includesVat }]));
  }

  const labourLines = labourRaw.map((line) => {
    const resolved = resolvedLabour.get(line.familyKey);
    return { familyKey: line.familyKey, id: resolved?.id ?? "", name: resolved?.name ?? line.name, qtyPerUnit: Number(line.qtyPerUnit), hourlyRate: Number(resolved?.hourlyRate ?? line.hourlyRate) * labourZoneFactor, currency: resolved?.currency ?? line.currency };
  });
  ...
  const equipmentLines = equipmentRaw.map((line) => {
    ...
    return { familyKey: line.familyKey, id: resolved?.id ?? "", name: resolved?.name ?? line.name, qtyPerUnit: Number(line.qtyPerUnit), hourlyCost: Number(resolved?.hourlyCost ?? line.hourlyCost) * equipmentZoneFactor, currency: resolved?.currency ?? line.currency };
  });
...
export type EffectiveCompositionResourcesV2 = {
  labour: Array<{ labourCategoryId: string; familyKey: string; name: string; hoursPerUnit: number; hourlyRate: number; currency: string }>;
  materials: Array<{ materialId: string; familyKey: string; name: string; unit: string; qtyPerUnit: number; unitCost: number; currency: string }>;
  equipment: Array<{ equipmentId: string; familyKey: string; name: string; hoursPerUnit: number; hourlyCost: number; currency: string }>;
};
...
export async function getCompositionEffectiveResourcesV2(
  ...
  const labour: EffectiveCompositionResourcesV2["labour"] = facts.labourLines.map((line) => ({ labourCategoryId: line.id, familyKey: line.familyKey, name: line.name, hoursPerUnit: line.qtyPerUnit, hourlyRate: line.hourlyRate, currency: line.currency }));
  const materialsOut: EffectiveCompositionResourcesV2["materials"] = facts.materialLines.map((line) => ({ materialId: line.id, familyKey: line.familyKey, name: line.name, unit: line.unit, qtyPerUnit: line.qtyPerUnit, unitCost: line.unitCost, currency: line.currency }));
  const equipmentOut: EffectiveCompositionResourcesV2["equipment"] = facts.equipmentLines.map((line) => ({ equipmentId: line.id, familyKey: line.familyKey, name: line.name, hoursPerUnit: line.qtyPerUnit, hourlyCost: line.hourlyCost, currency: line.currency }));
  ...
  for (const row of labour) { const cur = labourByFamily.get(row.familyKey); if (!cur) labourByFamily.set(row.familyKey, { ...row }); else { cur.hoursPerUnit += row.hoursPerUnit; cur.hourlyRate = row.hourlyRate; } }
  const materialByFamily = new Map<string, EffectiveCompositionResourcesV2["materials"][number]>();
  for (const row of materialsOut) { const cur = materialByFamily.get(row.familyKey); if (!cur) materialByFamily.set(row.familyKey, { ...row }); else { cur.qtyPerUnit += row.qtyPerUnit; cur.unitCost = row.unitCost; } }
  const equipmentByFamily = new Map<string, EffectiveCompositionResourcesV2["equipment"][number]>();
  for (const row of equipmentOut) { const cur = equipmentByFamily.get(row.familyKey); if (!cur) equipmentByFamily.set(row.familyKey, { ...row }); else { cur.hoursPerUnit += row.hoursPerUnit; cur.hourlyCost = row.hourlyCost; } }
