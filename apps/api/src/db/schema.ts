...
export const measurementCertificates = pgTable("measurement_certificates", {
  ...
  contractVariationId: uuid("contract_variation_id").references((): AnyPgColumn => contractVariations.id, { onDelete: "set null" }),
  number: integer("number").notNull(),
  periodStartDate: date("period_start_date"),
  periodDate: date("period_date").notNull(),
  status: documentStatusEnum("status").notNull().default("rascunho"),
  notes: text("notes"),
  /** Snapshot de retenção no momento da certificação — taxa effective-dated do contrato. */
  grossCertifiedAmount: numeric("gross_certified_amount", { precision: 14, scale: 2 }),
  retentionRateSnapshot: numeric("retention_rate_snapshot", { precision: 5, scale: 4 }),
  retentionAmount: numeric("retention_amount", { precision: 14, scale: 2 }),
  previousRetentionHeld: numeric("previous_retention_held", { precision: 14, scale: 2 }),
  netDueAmount: numeric("net_due_amount", { precision: 14, scale: 2 }),
  releasedRetentionAmount: numeric("released_retention_amount", { precision: 14, scale: 2 }).notNull().default("0"),
